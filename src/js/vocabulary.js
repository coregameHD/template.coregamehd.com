// Vocabulary page specific JavaScript
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updatePreview() {
        ...
        const romajiText = romajiTextInput ? romajiTextInput.value : '';
        ...
        let output = `【คำศัพท์】\n\n${japaneseText}`;
        if (romajiText) output += `\n${romajiText}`;
        if (thaiText) output += `\n${thaiText}`;
        if (selectedWordTypes) output += `\n\n※ ${selectedWordTypes}`;
    ...
    function setupEventListeners() {
        ...
        [japaneseTextInput, romajiTextInput, thaiTextInput, descriptionInput, gameTitleInput].forEach(input => {
            if (input) {
                input.addEventListener('input', updatePreview);
            }
        });

        // Add change event to JLPT exam checkbox
        if (jlptExamCheckbox) {
            jlptExamCheckbox.addEventListener('change', function() {
                ...
                if (this.checked) {
                    const nLevelValue = nLevelSelect ? nLevelSelect.value : 'None';
                    if (nLevelValue !== 'None' && jlptLevelSelect) {
                        jlptLevelSelect.value = nLevelValue;
                    }
                    if (yearInput) yearInput.value = new Date().getFullYear();
                    if (monthInput) monthInput.value = new Date().getMonth() + 1;
                }
                updatePreview();
            });
        }

        // Add change event to JTEST exam checkbox
        ...
        // Add change event to N-level select
        if (nLevelSelect) {
        ...
        // Add change event to JLPT level, year, month, and jtest-no inputs
        [jlptLevelSelect, yearInput, monthInput, jtestNoInput].forEach(input => {
            if (input) {
